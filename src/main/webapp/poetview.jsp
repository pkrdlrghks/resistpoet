        ...
        <div class="row g-0">
            ...
            <div class="card-body">
              ...
              <form action="/addreview" method="post" name="reviewForm">
              	<input type="button" value="후기등록" onclick="">
              	<input type="hidden" name="poetTitle" value="<%=request.getParameter("name")%>">
              	<input type="text" name="review"  id="reviewWriter" style="display: none;">
              </form>
              <table>
              <%
              	ArrayList<ReviewVO> vo=(ArrayList<ReviewVO>)request.getAttribute("reviewlist");
              	Iterator voIterator=vo.iterator();
              	while(voIterator.hasNext()){
              		ReviewVO reviewDater=(ReviewVO)voIterator.next(); %>
                <tr>
                  <td><%=reviewDater.getReview() %></td>
                  <td><%=reviewDater.getId() %></td>
                  <td><%=reviewDater.getWriteDate() %></td>
                </tr>
                <%} %>
              </table>
            </div>
